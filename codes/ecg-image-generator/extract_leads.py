...
from ecg_plot import ecg_plot, ecg_image_plot
import wfdb
...
def get_image_ecg(input_file,header_file, add_dc_pulse,add_bw,show_grid,add_print, start_index = -1,key='val',resolution=100,units='inches',papersize='',store_text_bbox=True,add_lead_names=True,pad_inches=1,standard_colours=5,full_mode='II',bbox = False,columns=-1):
    ...
    if(bw):
        grid_colour = 'bw'
    name, _ = os.path.splitext(full_header_file)
    rec_file = name       
    out_dict = ecg_image_plot(frame,full_header_file=full_header_file, style=grid_colour, sample_rate = rate,columns=columns,rec_file_name = rec_file,resolution = resolution, pad_inches = pad_inches, lead_index=full_leads, full_mode = full_mode, store_text_bbox = store_text_bbox, show_lead_name=add_lead_names,show_dc_pulse=dc,papersize=papersize,show_grid=(grid),standard_colours=standard_colours,bbox=bbox, print_txt=print_txt)
    return out_dict
...
if __name__=='__main__':
     ...
     result_list = get_image_ecg(input_file,header_file, bernoulli(1),bernoulli(0),bernoulli(1),bernoulli(1),bbox=True)